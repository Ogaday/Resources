...
for host in $(cat $host_list); do echo "checking host $host"; found=$(ssh wow203@$host "if [ -a /scratch/wow203/OpenFOAM/wow203-2.1.0/run/generation10/$1 ]; then echo 1; else echo 0; fi"); if [ $found -eq 1 ]; then echo "Found case $1 on host $host!"; loc=$host; found=0; fi; done

if [ "$loc" != "" ]; then echo "Found! On host $loc"; else echo "Not found"; fi
